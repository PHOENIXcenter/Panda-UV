###终端离子的匹配函数
#根据匹配上的理论离子和实验离子构造CM_output的某一列
from .ProteinClass import Clip,Ion,Mod
from .ion_match_utils import get_matched_index,construct_CM_series
import pandas as pd
from pyteomics import mass
#获取带N-氨基酸的理论离子碎片和实验离子碎片的匹配结果，以CM_output结果输出。允许输出重复的实验离子。
def get_Nterminal_CM_output(mono_mass_arr,protein,ion_type_list,ppm,unloc_mod_df):
    seqLen = protein.SEQLEN
    #保证即使没有匹配到离子仍然有输出。
    CM_output_template = pd.DataFrame(columns = range(12))
    start = 1
    #先匹配一遍不带非固定修饰的离子，再匹配非固定修饰的离子
    for end in range(1,seqLen):#end=[1,seqLen-1]--zhuyl,230614
        #end+=1
...
#获取带C-氨基酸的理论离子碎片和实验离子碎片的匹配结果，以CM_output结果输出
def get_Cterminal_CM_output(mono_mass_arr,protein,ion_type_list,ppm,unloc_mod_df):
    ...
    #先匹配一遍不带非固定修饰的离子
    for start in range(2,seqLen+1):#start=[2,seqLen]--zhuyl,230614
        #start+=1
        pep = Clip(protein).clip(start,end)
        for ion_type in ion_type_list:
            ion = Ion(pep).ionization(ion_type)
