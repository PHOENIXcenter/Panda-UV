...
def cal_PCC(iso_peak_arr, match_peak_arr):
    ...
    if (
        np.all(tmp2) == 0
    ):  # 理论分布可能匹配不到实验分布，实验分布的mz 和 int全是0，造成0除错误。遇到这种情况直接全部范围0。
        return 0
    PCC = (
        np.sum(tmp1 * tmp2)
        / np.sqrt(np.sum(np.square(tmp1)))
        / np.sqrt(np.sum(np.square(tmp2)))
    )
    return PCC
...
def iso_peak_match(iso_peak_arr, ms_peak_arr, peak_match_error):
    ...
    match_peak_arr[:, 1][missing_peak_index] = (
        0  # 超出ppm则认为没有匹配到峰，mz 和 int都设为0--zhuyl,230511
    )
    return match_peak_arr
...
def peak_scale(iso_peak_arr, match_peak_arr):
    ...
    if matched_peak_number >= 3:
        ...
        scale_factor = np.sum(y_dot_arr[yr_h_top_three_index]) / np.sum(
            yr_arr[yr_h_top_three_index]
        )
...
def cal_dx(iso_peak_arr, match_peak_arr, peak_match_error):
    ...
    if matched_peak_number == 0:
        dx = np.sqrt(
            np.sum(np.square(x_arr - cal_mz(x_arr, peak_match_error)))
            / len(iso_peak_arr)
        )
    else:
...
def cal_dy(iso_peak_arr_scaled, match_peak_arr):
    ...
    if sum(overlap_index) >= 1:
        dy_arr[overlap_index] = np.min(
            np.vstack(
                [
                    abs(yr_arr[overlap_index] - yr_dot_arr[overlap_index]),
                    np.array([t] * sum(overlap_index)),
                ]
            ),
            axis=0,
        )
    if sum(normal_index) >= 1:
        dy_arr[normal_index] = c * np.min(
            np.vstack(
                [
                    abs(yr_arr[normal_index] - yr_dot_arr[normal_index]),
                    np.array([t] * sum(normal_index)),
                ]
            ),
            axis=0,
        )
...
def get_score_term(ms_peak_arr_shift, ions_df_i, peak_match_error):
    # print(ions_df_i)
